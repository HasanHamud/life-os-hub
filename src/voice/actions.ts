  ...
  switch (intent.type) {
    ...
    case "ADD_TASK": {
      ...
      if (!intent.title || intent.title.trim().length === 0) {
        return { message: "I couldn't understand the task title." };
      }
      ...
      if (intent.date) {
        try {
          const dateStr = intent.date; // YYYY-MM-DD
          const timeStr = intent.time || "09:00"; // default time
    
          const full = new Date(`${dateStr}T${timeStr}:00`);
          if (!isNaN(full.getTime())) {
            deadline = full.getTime();
          }
        } catch (e) {
      ...
      const priority = intent.priority
        ? priorityMap[intent.priority] || "med"
        : "med";
    
      // 🏷️ Tags (optional)
      const tagIds: string[] = []; // extend later if you have tag system
      ...
      // 🗣️ Smart response
      let message = `Added task: ${intent.title}`;
    
      if (intent.date) {
        message += ` on ${intent.date}`;
      }
    
      if (intent.time) {
        message += ` at ${intent.time}`;
      }
